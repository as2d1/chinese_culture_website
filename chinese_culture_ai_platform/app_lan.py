...
def get_answer_stream(messages, response_queue):
    ...
    # 打印请求信息用于调试
    logger.info(f"正在向API发送请求，消息历史长度: {len(messages)}")
    for msg in messages:
        logger.debug(f"- {msg['role']}: {msg['content'][:30]}...")
...
if __name__ == '__main__':
    ...
    logger.info(f"本地访问地址: http://127.0.0.1:5000")
    logger.info(f"局域网访问地址: http://{local_ip}:5000")
    logger.info(f"请确保您的防火墙已允许5000端口访问")
